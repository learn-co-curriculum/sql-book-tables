...
Let's talk types. We've learned that when we create a table, we include a name for it, and that we also have to define at least one column at that time. We define columns in a `CREATE` statement by including a name, and a datatype to let SQLite know the kind of data it we will be storing there. The practice of explicitly declairing a type is known as "typing." On the surface, which is really as deep as we need to go right now, types are pretty simple to understand. `TEXT` stores plain old text, and `INTEGER` stores numbers. If that was all you knew about types you could get by decently with the rest of the material we are going to cover. For the sake of learning though, let's explore what typing is really about.
...
<p data-visibility='hidden'>View <a href='https://learn.co/lessons/sql-book-tables' title='Preface'>Preface</a> on Learn.co and start learning to code for free.</p>
